...
Configuration is loaded from an env file. Copy `.env.example` to a **gitignored** `.env.dev` (or `.env.prod`) and fill it in — only `.env.example` is committed. Example `.env.dev`:
